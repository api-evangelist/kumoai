...
- **Use `.venv/`, never system Python**: Reuse the project's `.venv/` if it exists with the right Python version (match the notebook kernel for `.ipynb` projects); otherwise create one with `python -m venv .venv` (or `python3 -m venv .venv` if `python` is unavailable). Run all commands through it (`.venv/bin/python`, `.venv/bin/pip`) — after venv creation, never invoke `python3` or `pip3` directly. Add `.venv/` to `.gitignore`.
- **Run commands as single, isolated steps**: Do not chain with `&&` or `||`, and avoid shell redirection (`>`, `|`, `2>/dev/null`). One command per step.
- **Non-trivial Python goes in a temp file**: Do not use multiline `python -c "..."`. For anything beyond a one-liner, write to `.claude_tmp/<name>.py` and run with `.venv/bin/python .claude_tmp/<name>.py`. Reuse temp files rather than creating duplicates.
- **Assume `.venv` exists**: Do not probe with `ls` before using it. If a command fails, diagnose and fix the root cause rather than adding pre-flight checks.
...
- **Default to pre-trained (RFM), not fine-tuned.** When the user says "predict X" without specifying, use pre-trained RFM (`kumoai.rfm` → `rfm.Graph`, `rfm.KumoRFM`, `model.predict()`). Only use fine-tuned (`kumoai` → `kumoai.Graph`, `kumoai.Trainer`, `trainer.fit()`) when the user explicitly asks for training or production deployment. Do not mix the two — they have different APIs, imports, and parameters (e.g., `lag_timesteps` works differently in each). When in doubt, check `context/guides/rfm-vs-training.md`.
- **Default to PQL + `model.predict()`, not `TaskTable`.** Always express predictions as PQL queries passed to `model.predict()`. Only fall back to `TaskTable` / `model.predict_task()` when the user has explicit context and prediction rows as DataFrames (e.g., a custom train/test split) that genuinely can't be expressed in PQL. PQL is simpler, more reproducible, and is the intended primary path.
